...
/// Sub-step epochs are formed as the START epoch plus an offset, never by
/// accumulating half-steps — time stays exact integer nanoseconds even inside
/// a single RK4 step.
...
void explicitEuler(StateVector& s, GravityField& field, Epoch t, double dt,
    ...
    // Position from the OLD velocity: on a curving path this consistently
    // overshoots outward, so the orbit gains energy on every step.
...
void semiImplicitEuler(StateVector& s, GravityField& field, Epoch t, double dt,
    ...
    // reordering makes the method symplectic and keeps the orbit closed.
    s.velocity += a * dt;
    s.position += s.velocity * dt;
}
...
void rungeKutta4(StateVector& s, GravityField& field, Epoch t, double dt,
    ...
    // Each stage asks where the planets are at a DIFFERENT instant, two of
    // them at the midpoint; the ephemeris answers exactly (docs/DESIGN.md §2).
