...
def main() -> int:
    ...
    cases = [
        Case("truecolour / half blocks / dimetric",
             ["--scenario", "empty", "--colour", "truecolour", "--blocks", "half", "--tilt", "30", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("truecolour / full cells",
             ["--scenario", "empty", "--colour", "truecolour", "--blocks", "full", "--tilt", "30", "--zoom", "4"],
             W, H, colour=True, half_blocks=False, max_bytes=60_000),
        Case("ansi16 / half blocks",
             ["--scenario", "empty", "--colour", "ansi16", "--blocks", "half", "--tilt", "30", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=40_000),
        Case("ascii density ramp",
             ["--scenario", "empty", "--colour", "ascii", "--tilt", "30", "--zoom", "4"],
             W, H, colour=False),
        Case("top-down (tilt 90)",
             ["--scenario", "empty", "--colour", "truecolour", "--tilt", "90", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("near edge-on (tilt 3)",
             ["--scenario", "empty", "--colour", "truecolour", "--tilt", "3", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("spun 137 degrees",
             ["--scenario", "empty", "--colour", "truecolour", "--tilt", "30", "--spin", "137", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("Earth-Moon, focused on Earth",
             ["--scenario", "empty", "--colour", "truecolour", "--tilt", "35", "--zoom", "2", "--focus", "Earth"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("everything, focused on Pluto",
             ["--scenario", "empty", "--colour", "truecolour", "--tilt", "30", "--zoom", "6", "--focus", "Pluto"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        Case("tiny terminal 40x12",
             ["--scenario", "empty", "--colour", "truecolour", "--size", "40x12", "--zoom", "4"],
             40, 12, colour=True, max_bytes=20_000),
        Case("wide terminal 200x50",
             ["--scenario", "empty", "--colour", "truecolour", "--size", "200x50", "--zoom", "4"],
             200, 50, colour=True, half_blocks=True, max_bytes=160_000),
        Case("far future date",
             ["--scenario", "empty", "--colour", "truecolour", "--date", "2049-12-31", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
        # One case per scenario, at its own default framing -- these render the
        # exact starting state the interactive binary shows, which is the only way
        # the harness says anything about what a user actually sees.
        Case("scenario: leo (the default)",
             ["--colour", "truecolour", "--tilt", "35"],
             W, H, colour=True, half_blocks=True, max_bytes=90_000),
        Case("ground-track map overlay",
             ["--colour", "truecolour", "--map", "--settle", "3600"],
             W, H, colour=True, half_blocks=True, max_bytes=90_000),
        Case("scenario: constellation",
             ["--scenario", "constellation", "--colour", "truecolour", "--tilt", "35"],
             W, H, colour=True, half_blocks=True, max_bytes=90_000),
        Case("scenario: transfer (settled past the burn)",
             ["--scenario", "transfer", "--settle", "800",
              "--colour", "truecolour", "--tilt", "35"],
             W, H, colour=True, half_blocks=True, max_bytes=90_000),
        Case("far past date",
             ["--scenario", "empty", "--colour", "truecolour", "--date", "1801-01-01", "--zoom", "4"],
             W, H, colour=True, half_blocks=True, max_bytes=60_000),
    ]
